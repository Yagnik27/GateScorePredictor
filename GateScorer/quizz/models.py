...
class ProgresssManager(models.Manager):
    ...
    def new_progresss(self, CPerson):
        new_progress = self.create(CPerson=CPerson,
                                   score="")
        new_progress.save()
        return new_progress
        
class Progresss(models.Model):
    CPerson=models.OneToOneField(CPerson,default="",on_delete=models.CASCADE,null=True)
...
class SittingManager(models.Manager):
    ...
    def new_sitting(self, CPerson, quiz):
        ...
        new_sitting = self.create(CPerson=CPerson,
                                  quiz=quiz,
    ...
    def user_sitting(self, CPerson, quiz):
        try:
            sitting = self.get(CPerson=CPerson, quiz=quiz, complete=False)
        except Sitting.DoesNotExist:
            sitting = self.new_sitting(CPerson, quiz)
        except Sitting.MultipleObjectsReturned:
            sitting = self.filter(CPerson=CPerson, quiz=quiz, complete=False)[0]
        return sitting

class Sitting(models.Model):
    CPerson = models.ForeignKey(CPerson, on_delete=models.CASCADE)
